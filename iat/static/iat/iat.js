window.IAT = (function(window, undefined) {
  ...
  function loadTasks() {
    ...
    var loadTask = function(taskIndex) {
      if (taskIndex < countTask) {
        ...
        return deferred.resolve(answerStore);
      }
    };
  ...
  function start(data) {
     ...
     * A block is a block with either a pair of concepts
    ...
    function createBlock(blockData) {
      ...
      return (function(blockData) {
        ...
        var currentTrial = 0;
        var preparedSet = prepareSets(setA, setB, blockData.order);
        var deferred = $.Deferred();
        var pressedBtn = null;
        ...
        var displayNextTrial = function() {
          ...
          if (preparedSet.length > 0) {
            ...
            // On each turn, pop a trial from the array.
        ...
        var startBlock = function() {
          console.log('[IAT] Starting block.');
     ...
     *
     * @param  {Object} setA  Following this nomenclatura: {type:String, items:Array}.
     * @param  {Object} setB  Following this nomenclatura: {type:String, items:Array}.
     * @param  {Array}  order Array with a patterns of A,B... suite explaining the order.
     * @return {array}  A stack of objects.
     */
    function prepareSets(setA, setB, order) {
      var args = [].slice.call(arguments);
      var result = [];

      function dequeue(arr) {
        var selected = arr.shift();
        arr.push(selected);
        return selected;
      }
      ...
      for (var i = 0; i < order.length; i++) {
        ...
        if (order[i] === 'A') {
          type = setA.type;
          item = dequeue(setA.items);
        } else {
          type = setB.type;
          item = dequeue(setB.items);
        }
